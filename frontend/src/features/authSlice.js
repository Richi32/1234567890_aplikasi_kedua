...
export const LoginUser = createAsyncThunk("User/LoginUser", async(user, thunkAPI) =>{
    try {
        const response = await axios.post('http://localhost:49156/login', {
            email: user.email,
...
export const getMe = createAsyncThunk("User/getMe", async(_, thunkAPI) =>{
    try {
        const response = await axios.get('http://localhost:49156/me');
        return response.data;
    } catch (error) {
        if(error.response){
            const message = error.response.data.msg;
...
export const LoginPesertaMagang = createAsyncThunk("PesertaMagang/LoginPesertaMagang", async(pesertamagang, thunkAPI) =>{
    try {
        const response = await axios.post('http://localhost:49156/login2', {
...
export const getMe2 = createAsyncThunk("PesertaMagang/getMe2", async(_, thunkAPI) => {
    try {
        const response = await axios.get('http://localhost:49156/me2');
        return response.data;
    } catch (error) {
        if(error.response){
            const message = error.response.data.msg;
...
export const LogOut = createAsyncThunk("User/LogOut", async() =>{
    await axios.delete('http://localhost:49156/logout');
})
